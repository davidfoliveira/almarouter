...
class AWSSQSClient extends EventEmitter {
  constructor(opts={}) {
    ...
    this.queueURL = this.opts.queueURL + "/x/";
  ...
  async _flushQueue() {
    ...
    console.debug(`DBUG: Flushing the sending queue with ${this.sendingQueue.length} in it...`);
    ...
    while (retries-- > 0) {
      console.info(`INFO: Sending ${queueSnapshot.length} message(s) to SQS...:`, command.input);
      try {
        res = await this.sqs.send(command);
        break;
      }
      catch(ex) {
        console.error(`ERRO: Error sending ${queueSnapshot.length} message(s) to SQS:`, ex.stack);
        if (retries) {
    ...
    if (!res) {
      console.info(`INFO: Failed to send ${queueSnapshot.length} message(s) to SQS and retries have been exhausted`);
      return null;
    }

    console.info(`INFO: Successfully sent ${queueSnapshot.length} message(s) to SQS`);
    return res;
  }
