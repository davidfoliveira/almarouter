...
class TCPServer {
  ...
  _acceptConnection(con) {
    ...
    con.on('data', (data) => {
      ...
      if (con._buf.length > this.maxBufferSize) {
        console.log(`WARN: Client '${remoteAddress}' is misbehaving. Disconnecting!`);
        con.destroy();
      }
    ...
    con.once('close', () => {
      console.log(`WARN: Connection from '${remoteAddress}' got closed.`);
      const endBuf = con._buf;
      if (endBuf.length > 0) this._cb(endBuf.toString());
      con._buf = Buffer.alloc(0);
    });
    con.on('error', (err) => {
      console.log(`ERROR: Connection error with client '${remoteAddress}': ${err}`);
      con._buf = Buffer.alloc(0);
      con.destroy();
    });
  }
  ...
  listen(cb) {
    ...
    this._server.listen(this.port, this.host, () => {
      console.log(`INFO: [changed]: Server listening to ${JSON.stringify(this._server.address())}`);
    });
  }
}
